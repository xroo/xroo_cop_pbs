...
{
	float3	e0s0		=  texCUBE( sky_s0, cubify_normals(vreflect)).rgb;
	float3	e0s1		=  texCUBE( env_s0, vreflect).rgb;
	float3 	e0s 		=  lerp(e0s0, e0s1, sphericalRoughness);
	float3	e1s0		=  texCUBE( sky_s1, cubify_normals(vreflect)).rgb;
	float3	e1s1		=  texCUBE( env_s1, vreflect).rgb;
	float3	e1s			=  lerp(e1s0, e1s1, sphericalRoughness);
...
)
{
	...
	float3	venv		=  nw;
	float3	e0d			=  texCUBE ( env_s0, venv ).rgb;
	float3	e1d			=  texCUBE ( env_s1, venv ).rgb;
...
)
{
	...
	float3	env_s 		=  get_skybox_reflection( skyRough, vreflect, env_color.w );
...
)
{
	...
	float3	hdiffuse, hspecular;
			...
			hdiffuse 		=  ambient_diff(nw,	hscale);
	...
	float3	dspecuilar		=  hspecular * (fresnelSchlickRoughness(NdotV, dielectric, roughness2) * brdf.x + brdf.y);
			dresult			=  dresult*dresult + dspecuilar*dspecuilar; // All the results have to be added squared
			dresult			*= occ;
			dresult			+= light.rgb;
